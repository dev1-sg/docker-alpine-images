...
variable "AWS_ECR_PUBLIC_IMAGE_URI" {
  default = "public.ecr.aws/dev1-sg/base/lambda-python:latest"
}
...
target "metadata" {
  labels = {
    ...
    "org.opencontainers.image.description" = "Lambda ${AWS_ECR_PUBLIC_IMAGE_NAME} ${AWS_ECR_PUBLIC_REPOSITORY_GROUP} image for internal use"
