...
func TestContainersGoPullAlpineCi(t *testing.T) {
	ctx := context.Background()
	container, e := testcontainers.GenericContainer(ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			Image: AlpineCi.AWS_ECR_PUBLIC_URI + "/" + AlpineCi.DOCKER_IMAGE_GROUP + "/" + AlpineCi.DOCKER_IMAGE + ":" + AlpineCi.DOCKER_IMAGE_TAG,
		},
	})
	require.NoError(t, e)
...
func TestContainersGoExecAlpineCi(t *testing.T) {
	ctx := context.Background()
	container, e := testcontainers.GenericContainer(ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			Image: AlpineCi.AWS_ECR_PUBLIC_URI + "/" + AlpineCi.DOCKER_IMAGE_GROUP + "/" + AlpineCi.DOCKER_IMAGE + ":" + AlpineCi.DOCKER_IMAGE_TAG,
